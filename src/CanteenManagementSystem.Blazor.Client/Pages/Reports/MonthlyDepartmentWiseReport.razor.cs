...
public partial class MonthlyDepartmentWiseReport
{
    ...
    {
        var url = $"/api/app/reports/excel/monthly-department-wise?from={Filter.From:yyyy-MM-dd}&to={Filter.To:yyyy-MM-dd}";
    ...
    protected void ExportCsv()
    {
        ...
        lines.AppendLine("Department,CC Code,Count,Total Amount");
        foreach (var r in Rows)
            lines.AppendLine($"\"{r.DepartmentName}\",\"{r.CCCode}\",{r.TransactionCount},{r.TotalAmount}");
        lines.AppendLine($"TOTAL,,,{Rows.Sum(r => r.TransactionCount)},{Rows.Sum(r => r.TotalAmount)}");
        DownloadCsv(lines.ToString(), $"monthly-dept-wise-{Filter.From:yyyyMM}.csv");
    }
