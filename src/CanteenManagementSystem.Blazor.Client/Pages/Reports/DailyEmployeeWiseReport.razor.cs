...
public partial class DailyEmployeeWiseReport
{
    ...
    protected async Task LoadDataAsync()
    {
        try
        {
            IsLoading = true;
            StateHasChanged();
            Rows = await ReportAppService.GetDailyEmployeeWiseAsync(Filter);
        }
        catch (Exception ex)
        {
    ...
    {
        var url = $"/api/app/reports/excel/daily-employee-wise?date={Filter.Date:yyyy-MM-dd}";
    ...
    protected void ExportCsv()
    {
        ...
        lines.AppendLine("Employee ID,Name,Department,Count,Total Amount");
        foreach (var r in Rows)
            lines.AppendLine($"\"{r.EmployeeIdNumber}\",\"{r.EmployeeName}\",\"{r.Department}\",{r.TransactionCount},{r.TotalAmount}");
        lines.AppendLine($"TOTAL,,,{Rows.Sum(r => r.TransactionCount)},{Rows.Sum(r => r.TotalAmount)}");
        DownloadCsv(lines.ToString(), $"daily-employee-wise-{Filter.Date:yyyyMMdd}.csv");
    }
